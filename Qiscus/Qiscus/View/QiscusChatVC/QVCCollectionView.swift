...
//                }
        ...
        case "deleteComment":
            ...
            break
        case "reply":
            if Qiscus.sharedInstance.connected && comment.type != .postback && comment.type != .account && comment.status != .failed && comment.type != .system && comment.status != .sending && comment.type != .card && comment.type != .contact{
                show = true
            }
            break
        case "forward":
